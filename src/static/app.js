      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length
          ? `<ul class="participants-list">${details.participants
              .map(
                (participant) =>
                  `<li><span>${participant}</span><button type="button" class="remove-participant" data-email="${encodeURIComponent(
                    participant
                  )}" aria-label="Remove participant" title="Remove participant">&times;</button></li>`
              )
              .join("")}</ul>`
